...
import { ArrowLeft, Star, Loader2, Trash2, Package, Download } from "lucide-react"
...
export default function GamePage({ params }: { params: { slug: string } }) {
  ...
  const [gameType, setGameType] = useState<"digital" | "physical">("digital")
  ...
  const buscarEndereco = async () => {
    ...
    setLoadingCep(true)
    try {
      const response = await fetch(`/api/buscar-cep?cep=${cep}`)
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      ...
      genre: game.genre,
      gameType: gameType,
      cep: gameType === "physical" ? cep : null,
      endereco: gameType === "physical" ? endereco : null,
    })
    setAdded(true)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <Card className="p-3 sm:p-4 bg-card/50">
              <h3 className="font-semibold mb-3 text-sm sm:text-base">Tipo de Jogo</h3>
              <RadioGroup value={gameType} onValueChange={(value) => setGameType(value as "digital" | "physical")}>
                <div className="flex items-center space-x-2 mb-2">
                  <RadioGroupItem value="digital" id="digital" />
                  <Label htmlFor="digital" className="cursor-pointer flex items-center gap-2">
                    <Download className="h-4 w-4" />
                    Jogo Digital (Download imediato)
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="physical" id="physical" />
                  <Label htmlFor="physical" className="cursor-pointer flex items-center gap-2">
                    <Package className="h-4 w-4" />
                    Jogo Físico (Entrega via correios)
                  </Label>
                </div>
              </RadioGroup>
              ...
              {gameType === "physical" && (
                <div className="mt-4 space-y-3">
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    />
                    <Button onClick={buscarEndereco} disabled={loadingCep} className="w-full sm:w-auto">
                      {loadingCep ? <Loader2 className="h-4 w-4 animate-spin" /> : "Buscar"}
                    </Button>
                  </div>

                  {endereco && (
                    <div className="p-3 bg-background rounded-lg border border-border text-xs sm:text-sm">
                      ...
                      <p className="text-muted-foreground mt-1">CEP: {endereco.cep}</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <Card className="p-4 sm:p-6 bg-card/50">
              <h3 className="font-semibold text-base sm:text-lg mb-3">Recursos Principais</h3>
